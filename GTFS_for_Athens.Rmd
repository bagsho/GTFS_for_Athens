...
After investigating the current files, it is decided to change the logic of the trips in the morning for a couple of lines. The reason for this is emanating from the fact that first buses in some lines are visiting the stops on their way to its morning starting point stops. The stop order information that were scraped from webpage is providing those stops as first stops of all the trips of day. However, in fact a better approach which is adopted here is to create separate trip information for first trips and group others based on their service types. In this section, all the required changes to make this transformation are implemented. 
