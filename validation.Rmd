...
First of all, since it can be imported with TidyTransit's read_gtfs function, it can be said that the feed is valid in general. Some feeds can't be even read by this function. In fact, in the first version of this feed, it couldn't be read by the function. After finding the incompatible columns and fixing it, the feed became readable.
